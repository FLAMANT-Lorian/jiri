        ...
        <section class="px-8 py-6">
            <div class="flex justify-between mb-4">
                ...
                <a class="self-end font-medium block px-6 py-2.5 rounded-xl bg-emerald-100 border border-emerald-200 hover:bg-emerald-200 hover:border-emerald-300 transition-all"
                   href="{{ route('contacts.edit', $contact->id) }}">Modifier le contact</a>
            </div>
            <div class="informations grid grid-cols-6 gap-6 mb-8">
                <div class="name bg-white p-6 col-span-2 rounded-2xl border border-gray-300">
                    <h3 class="text-lg font-normal pb-2">Nom</h3>
                    <p class="text-3xl font-semibold">{!! $contact->name !!}</p>
                </div>
                <div class="name bg-white p-6 col-span-3 rounded-2xl border border-gray-300">
                    <h3 class="text-lg font-normal pb-2">Email</h3>
                    <p class="text-3xl font-semibold">{!! $contact->email !!}</p>
                </div>
                <div class="flex flex-col justify-between bg-white p-6 col-span-1 col-start-6 row-span-2 rounded-2xl border border-gray-300">
                    <h3 class="text-lg font-normal pb-2 before:content-['⊙'] before:text-orange-700 before:pr-1">
                        Avatar</h3>
                    @if($contact->avatar)
                    ...
                    @else
                        <p class="italic pb-20 text-center">Aucun avatar séléctionné</p>
                    @endif
                </div>
                <div class="bg-white p-6 col-span-1 row-start-2 rounded-2xl border border-gray-300">
                    <h3 class="text-lg font-normal pb-2 before:content-['⊙'] before:text-orange-700 before:pr-1">
                        Jiris</h3>
                    <p class="text-3xl font-semibold">{!! $contact->jiris->count() !!}</p>
                </div>
                <div class="bg-white p-6 col-span-2 row-start-2 rounded-2xl border border-gray-300">
                    <h3 class="text-lg font-normal pb-2 before:content-['⊙'] before:text-orange-700 before:pr-1">
                        Nombre de fois évaluateurs
                    </h3>
                    <p class="text-3xl font-semibold">{!! $contact->jiris()->where('role', ContactRoles::Evaluators->value)->count() !!}</p>
                </div>
                <div class="bg-white p-6 col-span-2 row-start-2 rounded-2xl border border-gray-300">
                    <h3 class="text-lg font-normal pb-2 before:content-['⊙'] before:text-orange-700 before:pr-1">
                        Nombre de fois évalués
                    </h3>
                    <p class="text-3xl font-semibold">{!! $contact->jiris()->where('role', ContactRoles::Evaluated->value)->count() !!}</p>
                </div>
            </div>
            <article class="mb-8">
                <h3 class="text-3xl font-semibold mb-4">Jiris</h3>
                <table
                        ...
                        @foreach($contact->jiris as $jiri)
                            <tr>
                                ...
                                <td class="pr-6 py-4 text-gray-900">
                                    <p class="
                                        {!! ($role = $jiri->attendances()->where('contact_id', '=', $contact->id)->pluck('role')->first()) === ContactRoles::Evaluated->value ?
                                                'bg-emerald-200 text-green-900' : 'bg-blue-200 text-blue-900'!!}
                                        max-w-fit py-0.5 px-2 rounded-3xl">
                                        {!! __('labels-buttons.' . $role) !!}
                                    </p>
                                </td>
                            </tr>
                        @endforeach
